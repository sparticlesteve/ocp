...
@registry.register_trainer("profile_forces")
class ProfileForcesTrainer(BaseTrainer):
    """
    Trainer class for the Structure to Energy & Force (S2EF) task for
    MLPerf HPC with profiler annotations.
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.config["optim"]["max_epochs"]):
            ...
            profile_epoch = self.config["profile"].get("profile_epoch", -1)
            profile_steps = self.config["profile"].get("num_steps", 0)
            enable_nvtx = (self.config["profile"].get("enable_nvtx", True) and
                           epoch == profile_epoch)
            with torch.autograd.profiler.emit_nvtx(enabled=enable_nvtx):
                ...
                for i in range(skip_steps, len(self.train_loader)):
                    ...
                    if distutils.is_master() and epoch == profile_epoch:
                        if i == len(self.train_loader) - profile_steps - 1:
                            print('starting profiler')
                            torch.cuda.profiler.start()
                        elif i == len(self.train_loader) - 1:
                    ...
                    if (
                        ...
                        torch.cuda.synchronize()
                        timer_t1 = time.time()
                        print(f"step_time: {timer_t1 - timer_t0}, steps_per_sec: {timer_count / (timer_t1 - timer_t0)}")
                        timer_count = 0
